...
def update_slow_dens(rho_full, rho_guess):
    ...
    rho_full_bar_k = get_rFT(rho_full, r_) * weight_k
...
def free_energy_large(rho_s, rho_f, SG_m):
    ...
    #  unbalanced energy
    delta_rho_bar_k = get_rFT(rho_f - rho_s, r_) * weight_k
...
def free_energy_small(rho_f, rho_s):
    ...
    rho_bar_k = get_rFT(rho_f, r_) * weight_k
...
if __name__ == "__main__":
    ...
    # prepare grid space
    data = pandas.read_csv('/scratch/btb32/cdft-solvation/parameters/dcf_ck_HS_decompose.txt', skiprows=1, header=None, delim_whitespace=True).to_numpy() 
    k_, weight_k = data[:,0], data[:,1] 
